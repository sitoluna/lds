...
if(isset($_POST['action']) and $_POST['action'] == "insert"){
	...
	$rental = new rental();
	$nextIdRental = $rental->getNextIdRental($id_client);
	...
	if($errors == ""){
		$overlaped = overlapedDates(convertDate($date_start,"en"),convertDate($date_end,"en"), $appartment, $id_client, $nextIdRental);
		...
		if(!($overlaped["status"])){
			if(!($arrayOverlapedDatesPR["status"])){
				...
				$rental = new rental($id_client,$nextIdRental,$date_start,$date_end,$appartment,$num_pers,$time_entry,$time_exit,$deposit,$price,$comments,$electricity_start,$electricity_end,$water_start,$water_end);
				$rental->insert();
				header("location: fichaCliente.php?id=".$id_client."&msg=newRentalInsered");
			}else{
...
if(!$formularioEnviado){
	$date_start = convertDate(getDateToday(),"es");
	$date_end = convertDate(getDateToday(),"es");
	$appartment = "";
	$deposit = "";
	$price = "";
	$comments = "";
...
<div class="container">
	...
	<div class="main">
		...
		<div class="content">
			...
			<div class="item">
				<h1>Nuevo alquiler para <?php echo ucwords($client->name); ?></h1>
				<br/>
				<br>
				<form action="nuevoAlquiler.php" method="post" name="form01" id="form01">
					<input type="hidden" name="action" value="insert"/>
				  	<input type="hidden" name="id_client" value="<?php echo $id_client?>"/>
					<input type="hidden" id="id_rental" name="id_rental" value="<?php if(isset($_GET['id_rental'])) echo $_GET['id_rental']; ?>"/>
